...
*   **Fail-Closed Parsing**: Lines are trimmed, comments (`#`, `;`) and blank lines ignored, duplicate names collapsed to the first value, and malformed names, missing values, overlength values, embedded line breaks, masked-display characters, or characters OkHttp cannot transport (non-ASCII, control characters) are refused with a per-line reason shown under the editor.
...
*   **Secret Redaction**: Header values of 8+ characters are registered with `SecretRegistry`, so a gateway token echoed back in a provider error body is redacted exactly like an API key. A shorter value is not registered, so redaction covers tokens, not every possible value.
*   **Encrypted At Rest**: Each provider's header block is stored as an AES-256-GCM envelope in `ProviderCredentialStore` under its own Keystore alias and credential-ID authenticated data. `SettingsRepository` writes the encrypted record first and only then commits the config with the field stripped, so the block is not persisted in the settings file and is rehydrated when a request needs it. The two stores are ordered, not transactional: rollback is best-effort within the process, and a crash between them can leave both copies on disk, in which case the encrypted record wins on read. Blocks written by earlier builds are migrated into the Keystore on launch.
*   **Redirects Refused**: Requests to a custom endpoint do not follow redirects. OkHttp drops `Authorization` across origins but forwards arbitrary custom headers, so following one would hand a gateway token to whatever host the redirect names.
